    ...
    const query = useQuery(
        ...
        async () => {
            const res = await axios.get(
                `https://apis20231023230305.azurewebsites.net/api/BirdService/GetHighestAvgratingBirdService`
            );
    ...
    return (
        <div className="top-services">
            <div className="rank-container">
                ...
                <div data-aos="zoom-in" data-aos-duration="1500" className="grid grid-cols-4 gap-4">
                    {query.data.map((item, key) => (
                        <ServiceCardItem key={key} item={item} />
                    ))}
                </div>
            </div>
        </div>
