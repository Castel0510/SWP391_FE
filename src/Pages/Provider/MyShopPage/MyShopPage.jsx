                    ...
                    {currentItems.map((item, index) => (
                        ...
                        >
                            <div className="w-full">
                                <img className="rounded-t-lg w-full h-[240px]" src={item.imageURL} alt="" />
                            </div>
                            <div className="p-5 ">
                                <div>
                                ...
                                <div className="my-4 font-bold">{item.pricePerDay}$</div>
                                <div className="flex justify-end">
                                    <Link
                                        to={{ pathname: `/item-detail-page/${item.id}` }}
                                        state={{ item }}
                                        className="inline-flex items-center px-3 py-2 mb-10 mr-4 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                                    >
                                        Edit
                                    </Link>
                                    <div
                                        className="inline-flex items-center px-3 py-2 mb-10 text-sm font-medium text-center text-white bg-red-700 rounded-lg cursor-pointer hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
                                        onClick={() => handleDelete(item.id)}
                                    >
                                        Delete
                                    </div>
                                </div>
                                <div className="clear-both pb-5">
                    ...
                    {Array.from({ length: totalPages }, (_, index) => (
                        <button
                            ...
                            className={`mr-2 p-2 rounded-full ${
                                currentPage === index + 1 ? 'bg-blue-500 text-white' : 'bg-blue-200 text-blue-800'
                            }`}
