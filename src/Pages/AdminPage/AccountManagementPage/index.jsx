...
const AccountManagementPage = () => {
        ...
        <button
            ...
            className={`relative inline-flex items-center px-4 py-2 text-sm font-semibold focus:z-20 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 ${
                pageIndex === pageNumber
                    ...
                    : 'hover:bg-gray-300 text-gray-900 ring-1 ring-inset ring-gray-300'
            }`}
        >
            {pageNumber + 1}
        </button>
    ...
    const handleBanUser = useMutation(
        async (id) => {
            const res = await axios.put(`https://apis20231023230305.azurewebsites.net/api/User/BanUser?id=${id}`);
            return res;
        },
        {
            onSuccess: (data) => {
    ...
    const handleUnbanUser = useMutation(
        async (id) => {
            const res = await axios.put(`https://apis20231023230305.azurewebsites.net/api/User/UnbanUser?id=${id}`);
            return res;
        },
        {
            onSuccess: (data) => {
    ...
    return (
        <div className="flex flex-col justify-between h-full">
                            ...
                            {data.map((item, index) => (
                                <tr key={index} className="bg-white border-b hover:bg-emerald-100">
                                    ...
                                    <td className="px-6 py-4 text-gray-900">
                                        {item.role === 0 ? 'Customer' : 'Provider'}
                                    </td>
                                    <td className="px-6 py-4 text-gray-900">
                                        {item.email === 'string' ? '(Not update yet)' : item.email}
                                    </td>
                                    <td className="px-6 py-4 text-gray-900">
                                        {item.phoneNumber === 0 ? '(Not update yet)' : '0' + item.phoneNumber}
                                    </td>
                                    <td
                                        className={`px-6 py-4 ${item.status === 0 ? 'text-green-500' : 'text-red-500'}`}
                                    >
                                        ...
                                        <div className="flex gap-1">
                                            {item.status === 0 ? (
                                                <button
                                                    ...
                                                    title="Block"
                                                    onClick={() => handleBanUser.mutate(item.id)}
                                                >
                                                    <MdOutlineNoAccounts />
                                                </button>
                                            ) : (
                                                <button
                                                    ...
                                                    className="font-medium text-blue-600 hover:underline"
                                                    title="Unblock"
                                                    onClick={() => handleUnbanUser.mutate(item.id)}
                                                >
            ...
            <div className="flex justify-end mt-3">
                <button
                    className="relative inline-flex items-center px-2 py-2 text-gray-400 rounded-l-md ring-1 ring-inset ring-gray-300 hover:bg-gray-300 focus:z-20 focus:outline-offset-0"
                ...
                <button
                    className="relative inline-flex items-center px-2 py-2 text-gray-400 rounded-r-md ring-1 ring-inset ring-gray-300 hover:bg-gray-300 focus:z-20 focus:outline-offset-0"
                    onClick={() => handlePageChange(pageIndex + 1)}
