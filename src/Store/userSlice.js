...
export const loginUser = createAsyncThunk('user/loginUser', async (userCredentials) => {
    ...
    if (response.status === 200) {
        const id = user.Id;
        const infoResponse = await axios.get(`${LINK_API}/api/User/Info?id=${id}`);
...
export const createCustomer = createAsyncThunk('user/createCustomer', async (customerCredentials) => {
    const response = await axios.post(`${LINK_API}/api/Customer/Create`, customerCredentials);
    return response;
});

export const createProvider = createAsyncThunk('user/createUser', async (providerCredentials) => {
    const response = await axios.post(`${LINK_API}/api/Provider/Create`, providerCredentials);
    return response;
});

//update profile profile
export const updateProvider = createAsyncThunk('user/updateProvider', async ({ id, providerCredentials }) => {
    try {
        const response = await axios.put(`${LINK_API}/api/Provider/Update?id=${id}`, providerCredentials);
        return response;
    } catch (err) {
        console.log(err);
    }
